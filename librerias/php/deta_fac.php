	...
	$cod=$_GET['ced'];
 	$rs=$db->Execute("SELECT * FROM clientes WHERE cod_cli='$_GET[ced]'");	
		...
		while (!$rs->EOF)
		{
			...
			$iva=$db->Execute("SELECT * FROM iva ORDER BY id DESC");
			$iva1=$iva->fields(1);			
						
			$tb1.="<tr><td colspan=3 align=right >IVA ".$iva->fields(1)*100 ."%</td><td align=center>".$suma*$iva1." Bs.F</td></tr>";
			$total+=($suma*$iva1)+$suma;

	...
	$cod_pre_comp=$rs->fields(0); $rif=$rs->fields(1); $fec=$rs->fields(2);
 	...
 	$rs=$db->Execute("SELECT * FROM proveedor WHERE rif_prov='$rif'");	
		if ($rs->RowCount()>0)
		{
			...
			$dir=$rs->fields(2);
			$telf=$rs->fields(3);
			$res="Orden de Compra encontrada";
		}

	$rs=$db->Execute("SELECT * FROM det_ord_comp WHERE cod_det_ord_comp  ='$cod'");
while (!$rs->EOF)
	{
	...
	}else{$res="No existe numero de Orden de Compra";}
@header("location:../../bus_ord_comp.php?res=$res&rif=$rif&nom=$nom&dir=$dir&telf=$telf&mes=$mes&c=$c&ced=$ced&fec=$fec&tb=$tb&imp=$imp&tb1=$tb1&tb2=$tb2");
?>